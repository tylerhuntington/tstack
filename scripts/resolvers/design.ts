...
function shellToolPath(dir: string, binary: string): string {
  return dir.startsWith('~')
    ? `$HOME${dir.slice(1)}/${binary}`
    : `${dir}/${binary}`;
}
...
export function generateDesignSetup(ctx: TemplateContext): string {
  const designBin = shellToolPath(ctx.paths.designDir, "design");
  const browseBin = shellToolPath(ctx.paths.browseDir, "browse");
